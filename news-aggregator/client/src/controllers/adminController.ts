...
import { fetchExternalServers, fetchExternalServerDetails, updateExternalServer, addCategory } from "../api/adminApi";
...
export class AdminMenuController {
  ...
  async adminMenuFlow() {
    while (true) {
      console.log(`\nWelcome to the News Aggregator Admin Panel, ${this.user.username}!`);
      ...
      if (choice === 1) {
        // 1. View list of external servers and status
        try {
          const res = await fetchExternalServers();
          console.log("\nList of external servers:");
          res.data.servers.forEach((s: any, i: number) =>
            console.log(`${i + 1}. ${s.name} - ${s.status} - last accessed: ${s.lastAccessed || "Never"}`)
          );
        } catch (err: any) {
